...
import type { AppSettings, FieldConfig, TaskFieldSettings, SupplierFieldSettings } from "@/types/settings";
import { DEFAULT_SETTINGS } from "@/types/settings";
import { supabase } from "@/lib/supabase";

const STORAGE_KEY = "taskflow_settings";

// Deep merge: preserva defaults para chaves que não existam no salvo
...
interface SettingsContextType {
  ...
  isSaving: boolean;
  // Helpers rápidos para formulários
...
export function SettingsProvider({ children }: { children: React.ReactNode }) {
  ...
  // Persiste no Supabase (user_settings) — best effort, não bloqueia
  const persistToSupabase = useCallback(async (newSettings: AppSettings) => {
    try {
      ...
      // silencia — localStorage garante a persistência local
    } finally {
      setIsSaving(false);
    }
  }, []);

  // Carrega do Supabase ao montar (sobrescreve localStorage se mais recente)
  useEffect(() => {
    (async () => {
      try {
        const { data: { user } } = await supabase.auth.getUser();
    ...
    setSettings(prev => {
      const next: AppSettings = {
        ...
        tasks: {
          ...
          fields: {
            ...prev.tasks.fields,
            [key]: { ...prev.tasks.fields[key], ...patch },
          },
        },
      };
      persistToSupabase(next);
    ...
    setSettings(prev => {
      const next: AppSettings = {
        ...
        suppliers: {
          ...prev.suppliers,
          fields: {
            ...prev.suppliers.fields,
            [key]: { ...prev.suppliers.fields[key], ...patch },
          },
        },
      };
      persistToSupabase(next);
  ...
  const updateInterface = useCallback((patch: Partial<AppSettings["interface"]>) => {
    setSettings(prev => {
      const next: AppSettings = { ...prev, interface: { ...prev.interface, ...patch } };
      persistToSupabase(next);
      return next;
    });
  }, [persistToSupabase]);
  ...
  const updateSystem = useCallback((patch: Partial<AppSettings["system"]>) => {
    setSettings(prev => {
      const next: AppSettings = { ...prev, system: { ...prev.system, ...patch } };
      persistToSupabase(next);
      return next;
    });
  }, [persistToSupabase]);
  ...
  const value = useMemo(() => ({
    ...
    isTaskFieldVisible,
    isTaskFieldRequired,
    isSupplierFieldVisible,
    isSupplierFieldRequired,
  }), [settings, updateTaskField, updateSupplierField, updateInterface, updateSystem, resetToDefaults, isSaving, isTaskFieldVisible, isTaskFieldRequired, isSupplierFieldVisible, isSupplierFieldRequired]);
